		...
		if((isset($_POST["nomesrv"])) and (isset($_POST["ip"]))  and (isset($_POST["login"]))  and (isset($_POST["senha"])) and (isset($_POST["tipo"]))){
           ...
           if ($servidor_online) {
			...
			if($_POST['tipo'] == "full"){
				...
				$ssh->exec(" wget ".$site."/scripts/update/alterarlimite.sh  ");
                $ssh->output();
				$ssh->exec(" chmod 777 alterarlimite.sh ");
                $ssh->output();
				$ssh->exec(" wget ".$site."/scripts/update/criarusuario.sh  ");
				$ssh->output();
				$ssh->exec(" chmod 777 criarusuario.sh ");
				$ssh->output();
				$ssh->exec("wget ".$site."/scripts/update/remover.sh ");
				$ssh->output();
				$ssh->exec(" chmod 777 remover.sh ");
				$ssh->output();
				$ssh->exec("wget ".$site."/scripts/update/sshmonitor.sh  ");
				$ssh->output();
				$ssh->exec(" chmod 777 sshmonitor.sh ");
				$ssh->output();
				$ssh->exec(" wget ".$site."/scripts/update/KillUser.sh  ");
				$ssh->output();
				$ssh->exec(" chmod 777 KillUser.sh ");
				$ssh->output();
				$ssh->exec(" wget ".$site."/scripts/update/AlterarSenha.sh ");
				$ssh->output();
				$ssh->exec(" chmod 777 AlterarSenha.sh");
				$ssh->output();
				$ssh->exec(" wget ".$site."/scripts/update/usuarios.db ");
				$ssh->output();
