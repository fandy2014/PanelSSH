...
if(  (isset($_GET["op"])) && (isset($_GET["id_servidor"]))   ){
	...
	if($operacao == "reiniciar" ){
	   ...
	   if($updatando['site']==''){	   	        echo '<script type="text/javascript">';
	    ...
	    // Link do seu dominio Pservers
	    $link=$updatando['site'];
		$ssh->exec(" cd /root ");
	    $ssh->output();
		$ssh->exec(" rm alterarlimite.sh ");
	    $ssh->output();
		$ssh->exec(" wget ".$link."/scripts/update/alterarlimite.sh  ");
	    $ssh->output();
		$ssh->exec(" chmod 777 alterarlimite.sh ");
	    $ssh->output();
		$ssh->exec(" rm  AlterarSenha.sh ");
	    $ssh->output();
		$ssh->exec(" wget ".$link."/scripts/update/AlterarSenha.sh  ");
	    $ssh->output();
		$ssh->exec(" chmod 777 AlterarSenha.sh ");
	    $ssh->output();
		$ssh->exec(" rm criarusuario.sh ");
	    $ssh->output();
		$ssh->exec(" wget ".$link."/scripts/update/criarusuario.sh ");
	    $ssh->output();
		$ssh->exec(" chmod 777 criarusuario.sh ");
	    $ssh->output();
		$ssh->exec(" rm remover.sh ");
	    $ssh->output();
		$ssh->exec(" wget ".$link."/scripts/update/remover.sh ");
	    $ssh->output();
		$ssh->exec(" chmod 777 remover.sh ");
	    $ssh->output();
		$ssh->exec(" rm sshmonitor.sh ");
	    $ssh->output();
		$ssh->exec(" wget ".$link."/scripts/update/sshmonitor.sh  ");
	    $ssh->output();
		$ssh->exec(" chmod 777 sshmonitor.sh ");
	    $ssh->output();
		$ssh->exec(" rm KillUser.sh ");
	    $ssh->output();
		$ssh->exec(" wget ".$link."/scripts/update/KillUser.sh  ");
	    $ssh->output();
		$ssh->exec(" chmod 777 KillUser.sh ");
	    $ssh->output();
		$ssh->exec(" wget ".$link."/scripts/update/usuarios.db ");
	    $ssh->output();
